      ...
      <div className="container mx-auto p-8 lg:p-16 bg-white rounded-lg shadow-md">
        ...
        <section className="mb-10">
          <p className="text-lg text-gray-700 mb-4">
            Todo o conteúdo deste site, incluindo, mas não se limitando a, logotipos, textos, gráficos, imagens, vídeos, áudios, ícones, interfaces e software, é de propriedade exclusiva da <strong>{settings?.name || 'NGX Brasil'}</strong>, protegida pelas leis de direitos autorais, marcas comerciais e outras legislações aplicáveis, como a Lei de Direitos Autorais (Lei nº 9.610/1998) e o Código de Propriedade Industrial (Lei nº 9.279/1996), no Brasil.
          </p>

          <p className="text-lg text-gray-700 mb-4">
            Nenhuma parte deste site pode ser copiada, reproduzida, modificada, distribuída, transmitida, publicada, licenciada ou explorada comercialmente sem a permissão expressa da <strong>{settings?.name || 'NGX Brasil'}</strong>. A violação destes direitos poderá resultar em penalidades cíveis e criminais conforme a legislação vigente.
          </p>
        ...
        <section>
          ...
          <p className="text-lg text-gray-700">
            Para qualquer questão relativa à propriedade intelectual ou solicitações de autorização, entre em contato com a <strong>{settings?.name || 'NGX Brasil'}</strong> pelo e-mail <a href={`mailto:${settings?.email}`} className="text-blue-500 hover:underline">{settings?.email}</a>.
          </p>
        </section>
      </div>
      <GoogleMaps />
